...
def generate_launch_description():
    ...
    #ock_name = GetNamespacedName(namespace, 'standard_dock')

    # Calculate dock offset due to yaw rotation
    #ock_offset_x = RotationalOffsetX(0.157, yaw)
    #ock_offset_y = RotationalOffsetY(0.157, yaw)
    # Spawn dock at robot position + rotational offset
    #_dock = OffsetParser(x, dock_offset_x)
    #_dock = OffsetParser(y, dock_offset_y)
    # Spawn robot slightly clsoer to the floor to reduce the drop
    # Ensures robot remains properly docked after the drop
    #_robot = OffsetParser(z, -0.0025)
